...
{\bf Proposition 2.} Suppose operators $S,T:\mathbb R^d\to\mathbb R^d$ are $\gamma_S$ and $\gamma_T$ contractions, respectively. Then $S\circ T$ is a $\gamma_S\gamma_T$% Define $x^{2k+1} = S(x^{2k})$ and $x^{2k+2} = T(x^{2k+1})$ for $k\ge 0$. Then the iterates $\{x^k\}$ converge linearly with rate at most $\sqrt{\gamma_S\gamma_T}$. 
